...
const Chart = ({ tasks, dependencies }) => {
  ...
  useEffect(() => {
    ...
    dependencies
      .filter(
        (dep) => existingTaskIds.has(dep.from) && existingTaskIds.has(dep.to)
      )
      .forEach((dep) => {
    ...
    dagre.layout(g);
    const totalGraphWidth = g.graph().width;
    g.nodes().forEach((nodeId) => {
      const node = g.node(nodeId);
      node.x = totalGraphWidth - node.x;
    });
    ...
    container
      ...
      .data(
        g
          ...
          .filter(
            (d) =>
              g.node(d) &&
              g.node(d).x !== undefined &&
              g.node(d).y !== undefined
          )
      )
      .enter()
      .append("rect")
      ...
      .attr(
        "fill",
        (d) =>
          g.node(d).data.critical ? "#ff9900" : "rgba(188,186,186,0.3)"
      )
      .attr("stroke", "black");
    ...
    container
      ...
      .append("tspan")
      .attr("x", (d, i, nodes) =>
        d3.select(nodes[i].parentNode).attr("x")
      )
      .attr("dy", "1.2em")
      .text((t) => t);
